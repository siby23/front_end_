import React, { useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import { API_GET_ALL_CONTACT } from '../api/Contact_Api'

function Contact_details({ searchDetails }) {
    const [contactDtails, setContactDetails] = useState([])

    useEffect(() => {
        API_GET_ALL_CONTACT().then(({ data }) => {

            setContactDetails(data)
        })
    }, [])


    return (
        <>
            {contactDtails ?
                contactDtails.map((item) => {
                    return (

                        <div className='bg-slate-600 w-[50%] rounded-lg flex p-1 shadow-md shadow-slate-500/30 '>

                            <img className='rounded-full mt-1 w-24 h-20 ' src="data:image/jpeg;base64," alt="" />
                            <div className='flex mt-7 justify-around  w-full'>

                                <h1 className='text-orange-100'>{item.name}</h1>
                                <h1 className='text-orange-100'>{item.location}</h1>
                                <h1 className='text-orange-100'>{new Date(item.createdAt).toLocaleDateString('en-US')}</h1>
                                <Link to={`/profile/${item.id}`}>
                                    <button className='bg-slate-500 text-center h-6  rounded pl-1 pr-1'>View</button>
                                </Link>
                            </div>
                        </div>
                    )
                })
                : ""
            }

            {searchDetails ?

                searchDetails.map((item) => {
                    return (

                        <div className='bg-slate-600 w-[50%] rounded-lg flex p-1 shadow-md shadow-slate-500/30'>

                            <img className='rounded-full mt-1 w-24 h-20 ' src="data:image/jpeg;base64," alt="" />
                            <div className='flex mt-7 justify-around  w-full'>

                                <h1 className='text-orange-100'>{item.name}</h1>
                                <h1 className='text-orange-100'>{item.location}</h1>
                                <h1 className='text-orange-100'>{new Date(item.createdAt).toLocaleDateString('en-US')}</h1>
                                <Link to={`/profile/${item.id}`}>
                                    <button className='bg-slate-500 text-center h-6  rounded pl-1 pr-1'>View</button>
                                </Link>
                            </div>
                        </div>
                    )
                })
                : ""}

        </>
    )
}

export default Contact_details
